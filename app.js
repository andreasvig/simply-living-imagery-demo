...
function render(){
 ...
 const items=(mode==='animation'?data.animations:[...data.parallax].reverse()).filter(i=>format==='covers'?i.width<i.height:i.width>i.height);
...
try{const response=await fetch('manifest.json?v=14');if(!response.ok)throw Error('manifest');data=await response.json();readRoute();render();}catch{$('#content').innerHTML='<p class="loading">The collection couldn’t load. Please refresh to try again.</p>';}
